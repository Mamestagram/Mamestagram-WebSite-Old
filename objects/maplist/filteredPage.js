app.get("/maplist/:type/:mode/:filter&page=:page",
    (req, res, next) => {
    ...
    (req, res, next) => {
        if (req.params.filter === "ranked" || req.params.filter === "loved") {
            next();
        }
        else {
            res.redirect("/maplist/custom/any");
    ...
    (req, res) => {
        ...
        pool.getConnection((err, connection) => {
            ...
            async function processAsyncTask() {
                try {
                    const status = req.params.filter === "ranked" ? 2 : 5;

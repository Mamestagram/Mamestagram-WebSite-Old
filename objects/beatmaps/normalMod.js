app.get("/beatmaps/:setid/:id&mods=:mods", (req, res) => {
    pageName = "Beatmaps";
    pool.getConnection((err, connection) => {
        const query = new Array(3);
        postLog(err, req, res, `${pageName}/${req.params.setid}/${req.params.id}&${req.params.mods}`);
        ...
        async function processAsyncTask() {
            try {
                ...
                query[2] = await queryPromise(
                    ...
                    "OR priv = 4099) " +
                    "AND NOT grade = 'F';",
                    [req.params.setid, req.params.id, query[1][0].mode, req.params.mods]
                );

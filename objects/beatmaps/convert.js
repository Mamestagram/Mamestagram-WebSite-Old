app.get("/beatmaps/:setid&:mode/:id", (req, res) => {
    pageName = "Beatmaps";
    modeNum = getModeNum(req.params.mode);
    pool.getConnection((err, connection) => {
        const query = new Array(3);
        postLog(err, req, res, `${pageName}/${req.params.setid}&${req.params.mode}/${req.params.id}`);
        ...
        async function processAsyncTask() {
            try {
                ...
                query[2] = await queryPromise(
                    ...
                    "OR priv = 4099) " +
                    "AND NOT grade = 'F';",
                    [req.params.setid, req.params.id, modeNum]
                );
